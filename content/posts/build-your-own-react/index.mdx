...
> ⚠️ Wait! I'm still writing this post ⚠️
...
Using `textNode` instead of, for example, setting `innerText` will allow us to treat all elements in the same way later. Note also how we set the `nodeValue` like we did it with the `h1` title, it's almost as if the string had `props: {nodeValue: "hello"}`, right?
